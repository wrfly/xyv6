...
class DbTransfer(object):
    ...
    def push_db_all_user(self):
        ...
        if query_sub_when == '':
            return
        query_sql = query_head + ' SET w_n = '+ str(Config.node_number) +', u = CASE port' + query_sub_when + \
    ...
    def pull_db_all_user():
        ...
        conn = cymysql.connect(host=Config.MYSQL_HOST, port=Config.MYSQL_PORT, user=Config.MYSQL_USER,
                               passwd=Config.MYSQL_PASS, db=Config.MYSQL_DB, charset='utf8')
        cur = conn.cursor()
        cur.execute("SELECT port, u, d, transfer_enable, passwd, plan, enable FROM user")
        rows = []
    ...
    #修改下面的逻辑要小心包含跨线程访问
        for row in rows:
            if ServerPool.get_instance().server_is_run(row[0]) is True:
                if row[6] == 0:
                    #stop disable user
                    logging.info('stop server at port [%s] reason: disable' % (row[0]))
                    ServerPool.get_instance().del_server(row[0])
                elif row[1] + row[2] >= row[3]:
                    #stop out bandwidth user
                    logging.info('stop server at port [%s] reason: out bandwidth' % (row[0]))
                    ServerPool.get_instance().del_server(row[0])
                if ServerPool.get_instance().tcp_servers_pool[row[0]]._config['password'] != row[4]:
                    #password changed
                    logging.info('stop server at port [%s] reason: password changed' % (row[0]))
                    ServerPool.get_instance().del_server(row[0]) 
            else:
                if row[6] == 1 and row[0] != 0 and row[1] + row[2] < row[3]:
                    logging.info('start server at port [%s] pass [%s]' % (row[0], row[4]))
    ...
    def thread_db():
        ...
        while True:
            ...
            try:
                ...
                logging.warn('thread except:%s' % e)
            finally:
                time.sleep(15)

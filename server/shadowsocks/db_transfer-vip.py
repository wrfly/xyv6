...
class DbTransfer(object):
    ...
    def push_db_all_user(self):
        ...
        if query_sub_when == '':
            return
        query_sql = query_head + ' SET w_n = '+ str(int(Config.node_number)) +', u = CASE port' + query_sub_when + \
    ...
    def pull_db_all_user():
        ...
        conn = cymysql.connect(host=Config.MYSQL_HOST, port=Config.MYSQL_PORT, user=Config.MYSQL_USER,
                               passwd=Config.MYSQL_PASS, db=Config.MYSQL_DB, charset='utf8')
        cur = conn.cursor()
        cur.execute("SELECT port, u, d, transfer_enable, passwd, plan, enable, vip_end_time FROM user WHERE port != 0 AND plan != \'C\'")
        rows = []
        ...
        for row in rows:
            if ServerPool.get_instance().server_is_run(row[0]) is True:
                ...
                if row[1] + row[2] >= row[3]:
                    ...
                    logging.info('stop port [%s] reason: out bandwidth' % (row[0]))
                    ServerPool.get_instance().del_server(row[0])
                if ServerPool.get_instance().tcp_servers_pool[row[0]]._config['password'] != row[4]:
                    #password changed
                    logging.info('stop port [%s] reason: password changed' % (row[0]))
                    ServerPool.get_instance().del_server(row[0])
                ...
                if row[7] > int(time.time()) and row[1] + row[2] < row[3]:
                    logging.info('start port [%s] pass [%s]' % (row[0], row[4]))

...
class medoo
{
	...
	public function ban_user($table, $where)
	{
		return $this->exec('UPDATE "' . $table . '" SET enable=0 ,port = 0' . $this->where_clause($where));
	}
	public function active_user($table, $where)
	{
		return $this->exec('UPDATE "' . $table . '" SET enable=1 ' . $this->where_clause($where));
	}	
